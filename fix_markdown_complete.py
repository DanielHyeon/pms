#!/usr/bin/env python3
"""
Markdown 파일의 모든 linter 오류를 완전히 수정하는 스크립트
"""

import re
import os

def fix_markdown_complete(file_path):
    """Markdown 파일의 모든 linter 오류를 완전히 수정합니다."""
    ...
    while i < len(lines):
        ...
        if re.match(r'^#{1,6}\s+', line):
            # 헤딩 앞에 빈 줄 추가 (첫 번째 줄이 아니고 이전 줄이 빈 줄이 아닌 경우)
        ...
        # 3. 리스트 처리
            ...
            # 리스트 앞에 빈 줄 추가 (이전 줄이 빈 줄이 아니고 리스트가 아닌 경우)
            ...
            # 리스트 뒤에 빈 줄 추가 (다음 줄이 빈 줄이 아니고 리스트가 아닌 경우)
        ...
        # 4. 코드 블록 처리
            ...
            if line.strip() == '```' and i < len(lines) - 1:
                ...
                elif next_line.startswith('interface ') or next_line.startswith('type '):
                    fixed_lines.append('```typescript')
                elif next_line.startswith('def ') or next_line.startswith('class '):
                    fixed_lines.append('```python')
                elif next_line.startswith('public ') or next_line.startswith('private '):
                    fixed_lines.append('```java')
                else:
                    fixed_lines.append('```')
            else:
                fixed_lines.append(line)
...
def main():
    ...
    if os.path.exists(markdown_file):
        fix_markdown_complete(markdown_file)
